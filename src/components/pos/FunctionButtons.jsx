...
import RecallHoldModal from './RecallHoldModal'
import {
    ...
    <>
      <div style={{
        ...
        {SIDE_BUTTONS.map((btn, i) => {
          ...
          const label = btn.label === 'Hold Bill' && holdLoading ? 'Holding' : btn.label

          const handleClick = () => {
            if (btn.label === 'Clear All') {
              clearAll()
              return
            }
          ...
          return (
            <button
              ...
              type="button"
              onClick={handleClick}
              disabled={btn.label === 'Hold Bill' && holdLoading}
              style={{
                display: 'flex',
                flexDirection: 'column',
                alignItems: 'center',
                ...
                fontSize: 9,
                fontWeight: isPOS ? 800 : 600,
                lineHeight: 1.2,
                textAlign: 'center',
                cursor: holdLoading && btn.label === 'Hold Bill' ? 'wait' : 'pointer',
                transition: 'all 0.1s',
                opacity: holdLoading && btn.label === 'Hold Bill' ? 0.65 : 1,
                userSelect: 'none',
      ...
      {recallOpen && <RecallHoldModal onClose={() => setRecallOpen(false)} />}
    </>
  )
}
      ...
      {FEATURE_BUTTONS.map((btn, i) => {
        ...
        return (
          <button
            ...
            onMouseDown={e => {
              e.currentTarget.style.transform = 'scale(0.93) translateY(0)'
          ...
          >
            ...
            <span style={{ textAlign: 'center', lineHeight: 1.2, maxWidth: 66, wordBreak: 'break-word' }}>
              {btn.label}
            </span>
          </button>
        )

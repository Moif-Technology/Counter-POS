...
export default function LiteQtyKeypad({ selectedRowKey, qtyBuffer, onPressKey, onApply }) {
  return (
    <div style={{ marginBottom: 'clamp(4px, 1vh, 8px)' }}>
      <div style={{
        padding: 'clamp(6px, 1.2vh, 9px) 12px', borderRadius: 'var(--r-md)',
        border: '1.5px solid var(--brand-border)', background: 'var(--brand-bg)',
        fontSize: 12, color: 'var(--text-3)', marginBottom: 'clamp(4px, 1vh, 8px)',
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 'clamp(4px, 1vh, 8px)' }}>
        {NUMPAD_KEYS.map(k => {
          ...
          return (
            <button
              ...
              style={{
                padding: 'clamp(9px, 2.3vh, 22px) 0', borderRadius: 12,
                border: `1px solid ${isAction ? 'var(--red-border)' : 'var(--border)'}`,
                background: 'var(--surface-2)',
                color: isAction ? 'var(--red)' : 'var(--text-1)',
                fontSize: isAction ? 'clamp(12px, 2vh, 15px)' : 'clamp(16px, 2.6vh, 22px)', fontWeight: 700,
      ...
      <button
        ...
        style={{
          width: '100%', padding: 'clamp(9px, 1.8vh, 14px) 0', marginTop: 'clamp(4px, 1vh, 8px)', borderRadius: 'var(--r-md)',
          border: 'none', minHeight: 'clamp(38px, 6vh, 48px)',
